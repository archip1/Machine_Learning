...
# ╔═╡ 4d02ba2c-2d70-4bce-a35d-ccb509ddcb12
...
function loss(lm::AbstractModel, ce::CrossEntropy, X , Y)
	...
	# 	# count = 0
		...
		sum += (-(Y[i])) * log(sigmoid(θ[i])) - (1 - Y[i]) * log(1-sigmoid(θ[i]))
...
# t-test
# H null: μ1 = μ2
# H alternate: μ1 is not equal to μ2
...
# use a t test bcz it tells us if the models r similar. 
# significant = models have similar error
# insignificant test = models have different errors and one has less error or one has more. It will require more testing for better testing

# Blue part of histyogram seemd right skewed
# Orange is left skewed
# Therefore assumptions are slightly violated ans can not perform the t-test
...
function checkforPrerequisites(baseline_error::AbstractVector{Float64},
	...
	# sd = sqrt (sum (x-xbar))/N)
	mu_1 += mean(baseline_error)
	
	mu_2 += mean(learner_error)
	
	num=0.0
	for i in 1:length(baseline_error)
		num += ((baseline_error[i] - mu_1)^2)
	end
	std_1 = sqrt(num / (length(baseline_error) -1))
	
	num=0.0
	for i in 1:length(learner_error)
		num += ((learner_error[i] - mu_2)^2)
	end
	std_2 = sqrt(num / (length(learner_error) -1))
...
# If the p-value is less than the alpha which in this case is 0.05, then we reject the null hypothesis. If the experimenter decides that they want to the significance level of 99% which means the alpha value is 0.01, then if the p-value is less then 0.01, they reject the null hypothesis.

# This experiment gives us a p-value of 9.324582661562886e-9. Therefore there is very strong evidence against the null hypothesis and since it is less than both alpha values, we can reject the null hypothesis for both alphas (both 95% and 99% significance level). In conclusion, with the significance level of either 99% or 95%, we have sufficient evidence to conclud ethat the error for Logistic Regression is not equal to the error for Polynomial Logistic Regression.s
...
function tDistPValue(baseline_error::AbstractVector{Float64}, 		           learner_error::AbstractVector{Float64})
	...
	difference = baseline_error - learner_error
	avg=0.0
	...
	for i in 1:length(difference)
		std += ((difference[i] - avg)^2)
	end
	std = sqrt(std/(length(difference) - 1))
