...
Welcome to Pluto Notebooks! In this course you will be spending quite a bit of time working within these notebooks. These notebooks are an HTML/CSS/Javascript built interface for interacting with and working with Julia. They were inspiried by Jupyter, and you can learn more about the notebooks through [their github](https://github.com/fonsp/Pluto.jl). The notebooks are lightweight (with files that can be used by the normal julia interpretor), and track how notebook cells depend on eachother to re-run cells when their dependencies change.

The only code in this notebook which is not found in julia's base is the plotting code provided by [StatsPlots](https://github.com/JuliaPlots/StatsPlots.jl) and [Plots](http://docs.juliaplots.org/latest/). Please go read more about these packages, as they will be useful for you throught the course. Another package we use is [PlutoUI]() which contains a multitude of utilities for building interfaces in pluto notebooks (see below). For all the cells, some are hidden by default. To see hidden cells click on the eye to the top-left of the cell of interest.
...
student = (name="Archi Patel", email="archi1@ualberta.ca", ccid="ccid", idnumber=1234567)
...
A gaussian distribution with mean μ, standard deviation σ. Sample data from this distribution using sample(gd, n). You can also get the mean (mean), standard deviation (stddev), and variance (var) through their corresponding functions.
"""
...
"""
	...
	A gaussian distribution with mean μ, standard deviation σ. Sample data from this distribution using `sample(gd, n)`. You can also get the mean (`mean(gd)`), standard deviation (`stddev(gd)`), and variance (`var(gd)`) through their corresponding functions.
	"""
...
Below you will be implementing the sample mean, variance, and standard deviation of a dataset X. This dataset is guaranteed to be a vector of floating point numbers. We want the estimates of the variance to be unbiased using the following formula

```math
V(X) = \frac{\sum_{i} (X_i - \mathbb{E}[X])^2}{n-1}
```
...
function var(X::Vector{Float64})
	...
	for i in X
		difference = i - mean(X)
		numerator_val += (difference ^2)
		difference = 0
	end
	
	return numerator_val/(length(X)-1)
end
...
let # 4d, 4e
	gd = GaussianDistribution(0, sqrt(10))
	mean(sample(gd,30))
end
...
`plot_density` plots a histogram of the data passed through X and a density estimated through a kernel density algorithm (see implementation on [github](https://github.com/JuliaPlots/StatsPlots.jl) for more details).
...
let
	plt_1 = plot_density(X)
	plt_2 = plot_box_and_violin(X)
	plot(plt_1, plt_2)
end
