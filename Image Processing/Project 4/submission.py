...
def part1():
    ...
    # local_minima = np.zeros([h, w, 3]) #size initialization

    # for i in range(3):
    #     local_minima[:, :, i] = minimum_filter(DoG[:, :, i], size=37, mode='constant') #NEEED SIZE ARGUMENT

    # size = [26,33,38] tried diff sizes

    local_minima = minimum_filter(DoG, size=21) #NEEED SIZE ARGUMENT
    # local_minima[:, :, 1] = minimum_filter(DoG[:, :, 1], size=33, mode='constant') #NEEED SIZE ARGUMENT
    # local_minima[:, :, 2] = minimum_filter(DoG[:, :, 2], size=38, mode='constant') #NEEED SIZE ARGUMENT
    ...
    A = (DoG == local_minima) #binary image
...
def is_4connected(row, col, row_id, col_id):
    ...
    if ((abs(row - row_id) == 1 and abs(col - col_id) == 0)):
        return True
    elif (abs(row - row_id) == 0 and abs(col - col_id) == 1):
        return True
    else:
        return False
...
def iterativeMinFollowing(img, markers):
    ...
    while True:
        ...
        for row in range(h):
            for col in range(w):
                ...
                if markers_copy[row, col] != 0:
                    continue  # skip labeled pixels
                
                min_row, min_col = getSmallestNeighborIndex(img, row, col)
                
                # check if smallest neighbor has a nonzero value
                if markers_copy[min_row, min_col] != 0:
                    markers_copy[row, col] = markers_copy[min_row, min_col]
                
                elif markers_copy[row, col] == 0:
                    n_unmarked_pix += 1
        ...
        if n_unmarked_pix == 0:  # If no unlabeled pixels left, break the loop
            break
