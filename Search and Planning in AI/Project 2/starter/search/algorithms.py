...
class CBSState:
    ...
    def compute_cost(self):
        ...
        # iterate through all agents (k many agents)
        for i in range(self._k):
            cost, path = astar.search(self._starts[i], self._goals[i], self._constraints[i])
            
            # Store the cost and path in the CBS state
            self._paths[i] = path
            self._cost += cost
    ...
    def is_solution(self):
        ...
        for i in range(self._k):
            ...
            for state in path:
                state_time_tuple = (state, state.get_g())
                
                if state_time_tuple in state_time_set: 
                    return False, state_time_tuple
            
                state_time_set.add (state_time_tuple)
    ...
    def successors(self):
        ...
        if not sol:
            for i in range(self._k):
                ...
                for state in path:
                    if conflict_state == state and conflict_time == state.get_g():
...
class CBS():
    def search(self, start):
        ...
        while OPEN:
            ...
            is_solution, conflict = n.is_solution()
