...
class PlotResults:
    ...
    def plot_results(self, data1, data2, label1, label2, filename):
        ...
        _, ax = plt.subplots()
        ax.scatter(data1, data2, s=100, c=colors, alpha=0.5, cmap=plt.cm.coolwarm, zorder=10)
...
class FirstAvailable(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        # iterate through all domains to see find first tuple with a domain greater than 1
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) > 1:
                    return (i, j)
                
        # Every tuple has domain = 1
        return None
...
class MRV(VarSelector):
    ...
    def select_variable(self, grid):
        ...
        # find a cell that has the min domain, iteratinf thru all cells
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if min_domain > len(grid.get_cells()[i][j]) > 1:
...
class AC3:
    ...
    def pre_process_consistency(self, grid):
        ...
        # iterate through each cell, if the domain is a length of 1, add to Q list
        for i in range(grid.get_width()):
            for j in range(grid.get_width()):
                if len(grid.get_cells()[i][j]) == 1:
                    Q.append((i, j))

        # call the consistnecy function
        return not self.consistency(grid, Q)
    ...
    def consistency(self, grid, Q):
        ...
        while len(Q) != 0:
            ...
            variables_assigned, inconsistency_found = self.remove_domain_row(grid, i, j) 
            if inconsistency_found:
                return True
            for q in variables_assigned:
                Q.append(q)

            variables_assigned, inconsistency_found = self.remove_domain_column(grid, i, j)
            if inconsistency_found:
                return True
            for q in variables_assigned:
                Q.append(q)

            variables_assigned, inconsistency_found = self.remove_domain_unit(grid, i, j)
            if inconsistency_found:
                return True
            for q in variables_assigned:
                Q.append(q)
...
class Backtracking:
    ...
    def search(self, grid, var_selector):
        ...
        # if AC3().pre_process_consistency(grid):
        #     return None
        ...
        for d in grid.get_cells()[i][j]:
            if grid.is_value_consistent(d, i, j):
                ...
                inconsistent =  AC3().consistency(copy_grid, [(i, j)])
...
for p in problems:
    ...
    print('Original Sudoku Puzzle #{}'.format(count))
    # g.print()

    var_select = MRV()
    start_time = time.time()
    ac3.pre_process_consistency(g)
    result_g_mrv = backtrack.search(g, var_select)
    end_time = time.time()
    ...
    result_g_mrv.print()
    print('Is the current grid a solution (Using MRV)? {}\n'.format('Yes' if result_g_mrv.is_solved() else 'No'))

    var_select = FirstAvailable()
    start_time = time.time()
    ac3.pre_process_consistency(g)
    result_g_fa = backtrack.search(g, var_select)
    end_time = time.time()
    ...
    print('Runtime: ', end_time - start_time)
    result_g_fa.print()
    print('Is the current grid a solution (Using FA)? {}\n'.format('Yes' if result_g_fa.is_solved() else 'No'))
...
# PART 1 TESTING
# file = open('tutorial_problem.txt')
# problems = file.readlines()
# file.close()
# """
# Testing of FirstAvailable and MRV implementation using instance from tutorial_problem.txt.
# """
# for p in problems:
...
#     # Print the original grid on the screen
...
#     result_g = Grid()
#     bt_search = Backtracking()

#     # Test FirstAvailable() implementation
#     var_selector = FirstAvailable()

#     result_g = bt_search.search(g, var_selector)

#     print('FirstAvailable Puzzle')
#     result_g.print()
#     print('Is the current grid a solution (using FirstAvailable)? {}\n'.format('Yes' if result_g.is_solved() else 'No'))

#     # Test MRV() implementation
#     var_selector = MRV()
...
# """
# Testing of FirstAvailable and MRV implementation using instance from tutorial_problem.txt. GIVEN
# """
